...
public abstract class Game {
    ...
    protected Game() {
        ...
        Log.getDefault().debug("JOGL Version " + JoglVersion.getInstance().getImplementationVersion());
    }
    
    private void determineMaxSettings() {
        GLDrawableFactory fact = GLDrawableFactory.getDesktopFactory();
        GLProfile glp = GLProfile.getDefault();
        GLCapabilities glCaps = new GLCapabilities(glp);
        GLDrawable drawable = fact.createOffscreenDrawable(fact.getDefaultDevice(), glCaps, new DefaultGLCapabilitiesChooser(), 800, 600);
        GLContext cont = drawable.createContext(null);
        drawable.setRealized(true);
        cont.makeCurrent();
        GL2 gl = cont.getGL().getGL2();
        int[] arr = new int[16];
        Arrays.fill(arr, 0);
