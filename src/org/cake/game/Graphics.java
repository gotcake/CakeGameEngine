...
import org.cake.game.font.TextureFont;
...
public class Graphics {
    ...
    public static enum BlendingMode {
        ...
        BlendAlpha (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA),
        Add (GL_SRC_ALPHA, GL_ONE),
        Override (GL_SRC_ALPHA, GL_ZERO),
        InverseAlphaAdd (GL_ONE_MINUS_SRC_ALPHA, GL_ONE),
        Multiply (GL_DST_COLOR, GL_ZERO),
        Test (GL_ONE, GL_ONE_MINUS_SRC_COLOR),
        //Baller (GL_SRC_ALPHA_SATURATE, GL_ONE_MINUS_SRC_ALPHA),

        ;

        private static Stack<int[]> blendModeStack = new Stack<>();

        public static void glPushBlendMode(GL2 gl) {
            int[] arr = new int[2];
            gl.glGetIntegerv(GL_BLEND_SRC, arr, 0);
            gl.glGetIntegerv(GL_BLEND_DST, arr, 1);
            blendModeStack.push(arr);
        }

        public static void glPopBlendMode(GL2 gl) {
            int[] arr = blendModeStack.pop();
            gl.glBlendFunc(arr[0], arr[1]);
        }

        private int sfactor, dfactor;
        private BlendingMode(int s, int d) {
            sfactor = s;
            dfactor = d;
        }
    ...
    public static final int DEFAULT_CIRCLE_SEGMENTS = 60;

    private static iFont defaultFont = null;

    public static iFont getDefaultFont() {
        if (defaultFont == null) {
            defaultFont = new TextureFont("arial", 30);
        }
        return defaultFont;
    }
    ...
    private BlendingMode blendingMode = BlendingMode.BlendAlpha;
    ...
    void initGL() {
        ...
        gl.glBlendFunc(blendingMode.sfactor, blendingMode.dfactor);
    }
    ...
    public void setClearScreenColor(iColor color) {
