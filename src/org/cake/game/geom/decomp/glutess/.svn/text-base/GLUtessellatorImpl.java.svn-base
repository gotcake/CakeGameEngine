...
public class GLUtessellatorImpl implements GLUtessellator {
    ...
    float[] normal = new float[3];    /* user-specified normal (if provided) */
    float[] sUnit = new float[3];    /* unit vector in s-direction (debugging) */
    float[] tUnit = new float[3];    /* unit vector in t-direction (debugging) */

    /*** state needed for the line sweep ***/

    private float relTolerance;    /* tolerance for merging features */
...
//    private GLUtessellatorCallback callMeshData;
    ...
    private static final float GLU_TESS_DEFAULT_TOLERANCE = 0.0f;
...
//        callMesh = NULL_CB;
    ...
    public void gluTessProperty(int which, float value) {
        switch (which) {
...
/* Returns tessellator property */
    public void gluGetTessProperty(int which, float[] value, int value_offset) {
        switch (which) {
            case GLU.GLU_TESS_TOLERANCE:
/* tolerance should be in range [0..1] */
            ...
            case GLU.GLU_TESS_AVOID_DEGENERATE_TRIANGLES:
                ...
                break;
            default:
                value[value_offset] = 0.0f;
                callErrorOrErrorData(GLU.GLU_INVALID_ENUM);
                break;
        }
    } /* gluGetTessProperty() */

    public void gluTessNormal(float x, float y, float z) {
        normal[0] = x;
        normal[1] = y;
        normal[2] = z;
    }
...
//                return;
    ...
    private boolean addVertex(float[] coords, Object vertexData) {
        GLUhalfEdge e;
    ...
    private boolean addVertex(float x, float y, float z) {
        GLUhalfEdge e;
    ...
    private void cacheVertex(float[] coords, Object vertexData) {
    ...
    private void cacheVertex(float x, float y, float z) {
        if (cache[cacheCount] == null) {
            cache[cacheCount] = new CachedVertex();
        }
    ...
    public void gluTessVertex(float x, float y) {
        ...
        for (i = 0; i < 3; ++i) {
            a = coords[i+coords_offset];
            if (x < -Constants.MAX_TESS_COORD) {
                x = -Constants.MAX_TESS_COORD;
                tooLarge = true;
            }
            if (x > Constants.MAX_TESS_COORD) {
                x = Constants.MAX_TESS_COORD;
    ...
    public void gluTessVertex(float[] coords, int coords_offset, Object vertexData) {
        int i;
        boolean tooLarge = false;
        float x;
        float[] clamped = new float[3];
...
/*ARGSUSED*/
    ...
    void callCombineOrCombineData(float[] coords, Object[] vertexData, float[] weights, Object[] outData) {
