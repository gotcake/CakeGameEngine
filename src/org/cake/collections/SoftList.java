...
/**
 * An iCollection List with soft references.
 * Hold objects until the garbage collector decides to remove them to free up memory.
 * @author Aaron Cake
 */
public class SoftList<V> implements iCollection<V> {
    ...
    public SoftList(Collection<V> collection) {
        list = new ArrayList();
        for(V obj: collection)
            add(obj);
    }
    
    public SoftList(iCollection<V> collection) {
        list = new ArrayList();
        for(V obj: collection)
            add(obj);
    }
